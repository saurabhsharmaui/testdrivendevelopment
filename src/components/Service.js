  ...
  render() {
    ...
    if(this.props.data){
      ...
      var education = this.props.data.education.map(function(education){
        var titlePics= "images/"+education.title;
        return <div key={education.school}>
        
        <p>{education.description}</p>
        <img className="profile-pic"  src={titlePics} alt="Viser Solar" />
        <hr></hr>
        <b>{education.degree}</b>
        <p>{education.graduated}</p>
        <p>{education.graduated1?education.graduated1:''}</p>
        <p>{education.graduated2?education.graduated2:''}</p>
